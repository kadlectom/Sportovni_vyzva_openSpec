...
- `NAVRH_Slack_notifikace.md` — proposed Slack notifications (status: návrh, not all built yet)
...
New features and changes go through [OpenSpec](https://github.com/Fission-AI/OpenSpec) (`/opsx:propose` → `/opsx:apply` → `/opsx:archive` in Claude Code). See the "Working on this project" section in [CLAUDE.md](CLAUDE.md) for details. `openspec/specs/` is the living, growing source of truth for how the app currently behaves — it fills in one shipped change at a time, it isn't meant to duplicate the PRD.
